...
| `![[file.xlsx\|@svg:Sheet]]` | SVG 流程图 | 从 xlsx 读取节点/连线并渲染 |
...
支持两种用法：**手写 SVG** 或 **从 xlsx 数据文件自动生成流程图**（节点、颜色、文本、顺序、连线）。
...
与表格相同，数据放在同级 `excel/` 目录。Obsidian 写法：

| 写法 | 说明 |
|------|------|
| `![[flow.xlsx\|@svg:nodes]]` | 从 `nodes` 工作表读取节点，自动查找 `edges` 连线表 |
| `![[flow.xlsx\|@svg:]]` | 未指定表名时使用第一个工作表 |

组件写法：

```html
<svgUtils urls="网址工具等/excel/flow-demo.xlsx" sheetName="nodes"></svgUtils>
```

![[flow-demo.xlsx|@svg:nodes]]

#### xlsx 数据格式

**节点表**（工作表名可为 `nodes`、`节点` 或任意名称）：

| 列名（中英文均可） | 说明 |
|--------------------|------|
| 类型 / type | `node` 或 `节点`（可省略） |
| id / 编号 | 节点唯一 ID |
| 文本 / label | 显示文字 |
| 填充 / fill | 在 Excel 中设置该列**单元格背景色**（不读取文本） |
| 边框 / stroke | 在 Excel 中设置该列**单元格边框/背景色** |
| 文本色 / textColor | 在 Excel 中设置该列或「文本」列的**字体颜色** |
| 顺序 / order | 层级顺序（一列一层）；**相同 order 的节点在同一列上下并列** |
| x、y | 坐标（省略则按列自动排列） |
| 宽、高 / width、height | 节点尺寸 |
| 形状 / shape | `rect`、`rounded`（默认圆角矩形） |
| 方向 / direction | 首行数据下可填：`horizontal`（默认，从左到右分列）、`vertical`（从上到下） |

**前置节点表**（可选，工作表名 `prerequisites`、`前置`、`前置节点`）：

| 列名 | 说明 |
|------|------|
| id / 节点 / to | 当前节点 ID |
| predecessors / 前置节点 / from | 单个前置节点 ID（一行一条关系） |

示例：`review` 的前置为 `validate`；`end` 需要 `publish` 与 `archive` 两条前置时，写两行即可。

**无需添加分流、汇合、中转等辅助节点**，只写实际业务节点，前置关系保持一行一条（一对一）。

默认**从左到右按列展示**：`order` 相同者在同一列上下并列，单列节点（如「开始」）垂直居中；连线为曲线；根节点位于画布垂直中央。

`flow-demo.xlsx` 示例：`受理` 下分 `解析/扫描` 两支，各自延续到 `校验/补全` → `审核/通知/审计` → `发布/归档` → `结束`。

**连线表**（可选，工作表名 `edges`、`连线`）：
...
| 顺序 / order | 连线绘制顺序 |
| 文本 / label | 连线上的说明 |
| 边框 / stroke | 在 Excel 中设置线条颜色 |

连线优先级：**连线表** > **前置节点表** > 按顺序自动生成（相邻顺序组之间全连接）。

> 颜色请在 Excel 里直接设置单元格格式（填充色、字体色、边框色），不要在单元格中输入 `#RRGGBB` 文本。
...
| SVG 流程图 | `![[file.xlsx\|@svg:Sheet]]` | 是 |
| PDF | `![[file.pdf]]` | 是 |
| 视频 / 音频 | `![[file.mp4]]` / `![[file.mp3]]` | 是 |
| 附件 | `![[file.zip]]` 或 `[[file.zip]]` | 是 |
| 站内页 | `[[页面名]]` | 是 |
| Callout | `> [!note]` | 是 |
| SVG 手写 | `<svgUtils>...</svgUtils>` | 否 |
| SVG 数据 | `<svgUtils urls="..." sheetName="...">` | 否 |
